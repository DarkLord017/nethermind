...
public class HeaderValidatorTests
{
    ...
    {
        OverridableReleaseSpec preForkSpec = new(Byzantium.Instance) { IsEip8198Enabled = false };
        OverridableReleaseSpec postForkSpec = new(Byzantium.Instance) { IsEip8198Enabled = true, SlotDurationMs = Eip8198Constants.NewSlotDurationMs };
        ...
        bool result = _validator.Validate(_block.Header, _parentBlock.Header);
        if (result != expectedResult) Console.WriteLine("Warnings: " + string.Join(", ", _testLogger.LogList));
        Assert.That(result, Is.EqualTo(expectedResult));
    }
